...
resource "docker_container" "grafana" {
  ...
  env = [
    ...
    "GF_SERVER_ROOT_URL=https://${local.grafana_fqdn}",
  ]
  ...
  labels {
    label = "traefik.http.routers.${local.prefix}grafana.rule"
    value = "Host(`${local.grafana_fqdn}`)"
  }

  labels {
    label = "traefik.http.routers.${local.prefix}grafana.entrypoints"
    value = "websecure"
  }

  labels {
    label = "traefik.http.routers.${local.prefix}grafana.tls"
    value = "true"
  }

  labels {
    label = "traefik.http.routers.${local.prefix}grafana.tls.certresolver"
    value = var.traefik_cert_resolver
  }

  labels {
    label = "traefik.http.routers.${local.prefix}grafana.middlewares"
    value = "${local.prefix}authelia@docker"
  }

  labels {
    label = "traefik.http.services.${local.prefix}grafana.loadbalancer.server.port"
    value = "3000"
  }
